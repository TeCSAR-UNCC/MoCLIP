...
from models.motion_clip import MotionTransformer, encode_motion
from models.moCLIP import MotionTransformerv2
...
# Added for motion_clip
# from motion_clip import MotionTransformer  #:contentReference[oaicite:0]{index=0}
...
def load_trans_model(model_opt, pre_trained_path):
    t2m_transformer = MaskTransformer(code_dim=model_opt.code_dim,
                                      ...
                                      opt=model_opt)
    ckpt = torch.load(pjoin(pre_trained_path, "model/latest_backup.tar"),
...
if __name__ == '__main__':
    ...
    if opt.dataset_name == 't2m':
        ...
        opt.data_root = '/home/gmaldon2/research/momask-codes/dataset/HumanML3D'
        ...
        opt.data_root = '/home/gmaldon2/research/momask-codes/dataset/KIT-ML'
        opt.motion_dir = pjoin(opt.data_root, 'new_joint_vecs')
        opt.joints_num = 21
        radius = 240 * 8
        fps = 12.5
    ...
    if opt.dataset_name == "t2m":
        pre_trained_path = "./log/t2m/t2m/2024-02-14-14-27-29_8_GPT_officialTrans_2iterPrdictEnd"
    else:
        pre_trained_path = "./log/kit/2024-02-19-22-08-55_0_kit"
    
    trans_opt = get_opt(pjoin(pre_trained_path, "opt.txt"), device=opt.device)

    # res_model = load_res_model(res_opt)
    trans_opt.code_dim = vq_opt.code_dim
    t2m_transformer = load_trans_model(opt, pre_trained_path)
    
    # Create Motion encoder 
    t2m_transformer.clip_model.motion = MotionTransformerv1(opt.dataset_name).to(opt.device)
    # Add function for motion encoder
    t2m_transformer.clip_model.encode_motion = encode_motion.__get__(t2m_transformer.clip_model, type(t2m_transformer.clip_model))
    # Load MoCLIP weights
    kpt = torch.load(opt.motion_clip, map_location=opt.device)
    # Replace CLIP weights with MoCLIP
    t2m_transformer.clip_model.load_state_dict(kpt, strict=False)
